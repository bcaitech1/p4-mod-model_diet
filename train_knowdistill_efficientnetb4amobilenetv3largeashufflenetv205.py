...
from src.trainer_kd_t3_wandb import TorchTrainer
...
def train_kd(
    teacher1_pretrained: str,
    teacher2_pretrained: str,
    teacher3_pretrained: str,
    model_name: str,
    ...
    teacher1_model = EfficientNet_b4(num_classes=9)
    if os.path.isfile(teacher1_pretrained):
        teacher1_model.load_state_dict(torch.load(teacher1_pretrained))
        print("teacher1 pretrained model loaded.")
    teacher1_model.to(device)

    teacher2_model = Mobilenetv3_large(num_classes=9)
    if os.path.isfile(teacher2_pretrained):
        teacher2_model.load_state_dict(torch.load(teacher2_pretrained))
        print("teacher2 pretrained model loaded.")
    teacher2_model.to(device)

    teacher3_model = Model("configs/model/shufflenet_v2_05_base.yaml", verbose=True)
    if os.path.isfile(teacher3_pretrained):
        teacher3_model.model.load_state_dict(torch.load(teacher3_pretrained))
        print("teacher3 pretrained model loaded.")
    teacher3_model.to(device)
...
#    timm.create_model(model_name='resnetv2_101x1_bitm',pretrained=True,num_classes=9)
    ...
    wandb.log({"Model/Teacher1Model": teacher1_model.__class__.__name__})
    wandb.log({"Model/Teacher2Model": teacher2_model.__class__.__name__})
    wandb.log({"Model/Teacher3Model": teacher3_model.__class__.__name__})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", default="configs/model/mobilenetv3_7.yaml", type=str, help="model config"
    )
    parser.add_argument(
        "--data", default="configs/data/taco_6_small_continue.yaml", type=str, help="data config"
    )
    parser.add_argument(
        "--teacher1_pretrained", default="expsaves/efficientnetb4_pretrained_64_2021-06-12_16-01-21/best.pt",
        type=str, help="to load teacher1(efficientnetb4) pretrained weight"
    )
    parser.add_argument(
        "--teacher2_pretrained", default="expsaves/mobilenetv3_large_pretrained_64_2021-06-12_12-28-25/best.pt",
        type=str, help="to load teacher2(mobilenetv3 large) pretrained weight"
    )
    parser.add_argument(
        "--teacher3_pretrained", default="expsaves/shufflenetv205_pretrained_64/shufflenetc2x0.5_238.pt",
        type=str, help="to load teacher3(mobilenetv2 x0.5) pretrained weight"
    )
    parser.add_argument(
        "--student_pretrained", default="expsaves/mobilenetv3_7_2021-06-15_10-53-41/best.pt",
    ...
    test_loss, test_f1, test_acc = train_kd(
        teacher1_pretrained=args.teacher1_pretrained,
        teacher2_pretrained=args.teacher2_pretrained,
        teacher3_pretrained=args.teacher3_pretrained,
        model_name=model_name,
