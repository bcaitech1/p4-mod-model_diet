...
from src.decomposer import tucker_decomposition_conv_layer, decompose, tucker_decomposition_conv_layer_VBMF
...
#    timm.create_model(model_name='resnetv2_101x1_bitm',pretrained=True,num_classes=9)
    ...
    # decompose
    model_instance.model[0].conv = tucker_decomposition_conv_layer_VBMF(model_instance.model[0].conv)
        ...
        for j in range(len(model_instance.model[i])):
            for k in range(len(model_instance.model[i][j].branch1)):
                if type(model_instance.model[i][j].branch1[k]) == nn.Conv2d and \
                    ...
                    model_instance.model[i][j].branch1[k] = tucker_decomposition_conv_layer_VBMF(
            ...
            for k in range(len(model_instance.model[i][j].branch2)):
                if type(model_instance.model[i][j].branch2[k]) == nn.Conv2d and \
                    ...
                    model_instance.model[i][j].branch2[k] = tucker_decomposition_conv_layer_VBMF(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", default="configs/data/taco_4_small.yaml", type=str, help="data config"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--student_pretrained", default="expsaves/shufflenetv205_pretrained_64/shufflenetc2x0.5_238.pt",
        type=str, help="to load student pretrained weight"
    )
    # parser.add_argument(
